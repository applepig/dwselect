...
| `small-appliance` | 小家電 | 清潔、空氣調節、照明、衛浴等非廚房小型家電 |
| `kitchen-appliance` | 廚房家電 | 冰箱、冷凍櫃、洗碗機、電子鍋、微波爐、烤箱等廚房電器 |
| `major-appliance` | 大家電 | 洗衣機、冷氣等非廚房大型家電 |
...
- PChome 商品頁可能回 429；遇到時先改用 PChome 商品 API 查名稱、價格與圖片，例如 `https://ecapi.pchome.com.tw/ecshop/prodapi/v2/prod/{PRODUCT_ID}&fields=Id,Name,Nick,Price,Pic,Slogan,Describe,Spec&_callback=jsonp`
- PChome API 也失敗、或頁面需要互動確認時，再使用 agent-browser 打開頁面檢查，不要用猜的補資料
...
Content-only 任務只驗證格式、schema 可讀、taxonomy reference、圖片與 generated artifacts。不要把目前資料集的商品數量、特定商品 ID、特定搜尋文件數寫進測試；這些是 CMS data，不是 logic。
