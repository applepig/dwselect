...
- 單一 Nuxt 4 SSG app，不是 monorepo；主要 app entrypoint 在 `app/`，Nuxt Content 設定在 `content.config.ts`。
...
- Google Sheets TSV importer `scripts/migrate-google-sheet-products.ts` 是 deprecated legacy-only，不可用來建立新版內容。
...
- 內容或 taxonomy 變更後要跑 `pnpm build:search-index`，或直接跑 `pnpm generate` 讓 index 與 SSG 一起更新。
