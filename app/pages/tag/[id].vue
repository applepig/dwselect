...
const tag_id = (Array.isArray(raw_id) ? raw_id[0] : raw_id) ?? ''
...
const canonical_url = getCanonicalUrl(`/tag/${tag_id}`)
...
const taxonomy_page_data = await useTaxonomyPageData('tag', tag_id)

// 三型別全空（或 tag id 不存在）→ 該路由不 prerender，直接訪問須主動丟 404，對齊 products/[id].vue。
