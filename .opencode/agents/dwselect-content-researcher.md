...
Your default mode for a task with a target JSON path is implementation：research the one assigned item，then create or update that target JSON file yourself（write the complete file，not just notes for someone else to type in），then report exactly what changed and what still needs coordinator audit and editorial。The coordinator's role is audit and editorial，not first-draft data entry——so you own the first draft of every agent-owned field，including a draft `title` for guides and a content-derived guide `short_description`。If the coordinator explicitly says research-only，do not modify files。
...
- Explicit exception to the「no filesystem inspection」rule：you MAY and MUST read two schema-truth references before writing——the generated JSON Schema under `content/.schema/`（`product.schema.json` / `guide.schema.json` / `link.schema.json`）and the taxonomy id lists under `content/taxonomies/`。These are the schema SSOT，not repo-state snooping。Read them with the allowed `jq`（see the mandatory pre-write step below）。
- For products，do not write or rewrite `short_description` or `long_description`。Those are user-authored personal opinions——leave them as provided（empty string when none）。**絕對不要把使用者的一句話拆成 `short_description` 一半、`long_description` 一半**：UI 擇一顯示（卡片與搜尋建議只讀 `short_description`，詳情頁的「DW 怎麼說」用 `long_description || short_description`），拆兩半會讓兩個畫面都殘缺。使用者只給一句話時，`short_description` 放完整原文，`long_description` 放相同字串或留空 `""`；使用者給了短句 + 補充時，`long_description` 必須是「短句原文 + `\n\n` + 補充」而非只有補充那半。For guides，`title` and `short_description` are content-derived：write a concise `title` and an objective 1-2 sentence `short_description` that states what the source post covers and its core takeaway，without inventing personal opinions or using subjective recommendation words（「便宜」「好用」「剛好」）。The coordinator edits wording afterward。
...
- 幣別跟著通路走：`amazonjp` → `JPY`（`price_text` 用全形 `￥` 前綴，例如 `￥3800`）、`amazonus` → `USD`（`$` 前綴）、台灣通路（`pchome`／`momo`／`costco`）→ `TWD`。**⚠️ Amazon 會依 IP 自動換成當地貨幣**——從台灣連線時日亞與美亞都會顯示 `TWD 520.39` 這種台幣換算值，照抄就會把當下匯率寫進 JSON，隔天過時還留下難讀小數。**解法是帶 `i18n-prefs` cookie 強制原幣別**（實測有效）：美亞 `curl -b 'i18n-prefs=USD'`、日亞 `curl -b 'i18n-prefs=JPY'`，主價格在 `id="corePrice_feature_div"` 內的 `a-offscreen`。寫入前自我檢查幣別符號與 `channel_id` 相符；出現 TWD／NT$ 就是抓錯，帶 cookie 重抓，不要用匯率回推。既有 9 筆日亞／美亞 offer 曾存台幣換算值，已於 sprint 043／044 修回原幣別（僅 corsair 因美亞下架無現價而保留待決），不要當範本。**以上只約束你自己抓來的價格**——使用者明確提供的 `price_text` 依下一條規則優先，即使是台幣換算值也不要覆寫，改為回報幣別不符與你抓到的原幣別價格。
...
- Do not add categories、channels、brands、or tags。If a missing brand/tag is useful，return a `taxonomy_suggestions` section with proposed IDs and rationale，and leave the target JSON using only existing taxonomy IDs。Never invent an id（e.g. `electronics`、`official`、`monitor`）——every `category_id`、`tag_ids[]`、and `offers[].channel_id` MUST be copied verbatim from the jq listing below。An id absent from the taxonomy files fails the content gate。
...
- Mandatory pre-write step——read the schema truth with jq, do not hand-guess the shape：
  ```bash
  # 必填欄位（含 slug、unpublished_at、archived_at、offers[].checked_at 等易漏欄位）
  jq '{required, offers_required: .properties.offers.items.required}' content/.schema/product.schema.json
  # 合法 taxonomy id（category_id / tag_ids / offers[].channel_id 只能從這裡抄）
  jq -r '.items[].id' content/taxonomies/{categories,tags,brands,channels}.json
  ```
  Every key listed in the schema's `required` array must be present in the file you write。Omitting any（the previous failure omitted `slug`、`unpublished_at`、`archived_at`、`offers[].checked_at`）breaks the content gate。
- Products must NEVER set `image_url`——use a local `image_file`，and set `image_url: null`（this holds for `draft` too，not only published；the rule lives in a zod refine that the JSON Schema cannot express，so it will not show up as「required」but is still enforced）。Non-published items may leave `image_file: null`，but `image_url` must still be `null`。
- New site-bound content is created publish-ready：`status: "published"` + `published_at` + a downloaded local `image_file` that passes the guard。Create `draft`／`unpublished`／`archived` ONLY when the coordinator explicitly assigns that status。A `draft` is invisible on the public site（filtered by `isPublished`），so a half-finished draft delivers nothing in a go-live PR。Never downgrade to `draft` just to dodge the image requirement or empty-opinion fields——if you cannot obtain a guard-passing local image or hit any hard blocker，report it as blocked（with candidate image URLs / dimensions）in your notes instead of silently committing a draft。
...
- For products，preserve `short_description`、`long_description`、`offers[].url`、and user-provided `price_text` unless explicitly instructed otherwise；for new products with no user-provided opinion，set `short_description` / `long_description` to empty string。
- For guides，you own `title` and a content-derived `short_description`（see above）；keep `source_url` as the post URL（strip a trailing `?`）。
