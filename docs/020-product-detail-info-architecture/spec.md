...
- [ ] M2：補 build-time payload mapper 與型別，產生 frontend-ready `cards`、`details_by_id`（含已排序 `related_products`）、navigation counts／resource rows；`ProductCardView`／`ProductDetailView`／`RelatedProductCardView` 三個獨立 model。
- [ ] M3：改 Nuxt runtime composables／pages 使用 frontend-ready payload，移除 runtime product view model mapping 與 placeholder（`description: null`／`purchase_link: ''`）；統一 content payload 的 `useAsyncData` cache key。
- [ ] M4：build script 去重——`build-public-discovery.ts` 與 `build-search-index.ts` 共用單次 `readPublicContentSource()` 輸出。
- [ ] M5：重排 `ProductDetail` UI 資訊架構與 back button 樣式。
- [ ] M6：更新 unit／smoke tests，鎖定欄位語意與避免 `dw_says`／`description` 回歸、避免共用 helper 再度分岔。
- [ ] M7：執行 `pnpm test`、`pnpm lint`、`pnpm typecheck`、必要時 `pnpm generate`，並用 agent-browser 驗收 mobile／desktop detail page。
