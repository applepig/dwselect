...
### 2026-06-09 根因確認：iOS Safari View Transition，suppress experimental.viewTransition

- **更正先前誤判**：本檔上方多條把 `@nuxt/ui/dist/runtime/plugins/colors.js` 當兇手的紀錄（含「移除 colors runtime plugin」「unhead v2/v3 mismatch」「雙 dev server 互相干擾」）皆為打在症狀上的誤判，已驗證並推翻：
  - 依賴樹乾淨：`pnpm why` 顯示 `vue@3.5.35`、`@vue/runtime-core@3.5.35`、`unhead`/`@unhead/vue@2.1.15` 全收斂，無重複實例，排除版本衝突。
  - colors.js 只是 canary：讀 Nuxt `useHead` wrapper（`nuxt/dist/head/runtime/composables.js`）確認 server 端走 `ssrContext.head` 永不報錯，只有 **client hydration** 在 `runWithContext` 內 `hasInjectionContext()` 為 false 時，`injectHead` 回 `undefined`、`@unhead/vue` 再 fallback 失敗才丟 `useHead() was called without provide context`。colors.js 只是 app init 第一個呼叫 `useHead()` 的 plugin；GPT 用 `app:resolve` hook 砍掉它後，爆點只是從 unhead 移到 Vue render（`currentRenderingInstance.ce`），證明根因更深。
  - 雙 server 非直接成因：`/app` 容器 dev server 與本 repo 走不同 Traefik host / network namespace，瀏覽器只連本 repo `:3000`，不會互餵 bundle。
- **真正根因**：`experimental.viewTransition: true` 在 **iOS Safari** 上不穩。Nuxt 把 router page transition 包進 `document.startViewTransition`，可能在 hydration 視窗內觸發 transition；iOS Safari 的 View Transition API 本身有 abort / crash bug，導致 hydration 壞掉。`useHead without provide context` 與 `currentRenderingInstance.ce` 都是這個 client hydration 失敗的下游症狀。先前所有「乾淨通過」驗證都跑在 Chromium / agent-browser / curl，**從未在 iPad Safari 驗 hydration**，是反覆鬼打牆的元兇。
- **隔離驗證**：還原 GPT 的 colors workaround（`variables.css`、`nuxt.config.ts` 的 `app:resolve` hook、相關 nuxt-smoke 測試）→ 設 `experimental.viewTransition: false` → 清 `.nuxt`/`.output`/Vite cache → 以 `nuxt dev --host 0.0.0.0 --port 3000`（非 `pnpm dev` 的 `--host ::`）冷啟動單一 dev server。使用者於 **iPad Safari 全新分頁**實測：hydration 穩定、商品頁來回操作無錯誤。一翻一回坐實 view transition 為唯一觸發點。
- **手刻無法規避（已查證）**：跨框架證據顯示問題在 WebKit `startViewTransition` 層級，非 Nuxt 包裝——React（#35336 enter/exit hard-crash iOS Safari）、SvelteKit（#10835 iOS Safari crash、#13169 需 `hasUAVisualTransition` 避開原生 swipe 疊加）、Astro（#8301/#8625 Safari fallback 壞、iOS 捲動 stutter）、Angular（#62491 同步雙呼叫 Safari abort 丟 `AbortError`）。手刻只能更細地 gate「不在危險條件觸發」，無法免疫 WebKit crash bug。
- **決策**：公開站行動 Safari 受眾大，view transition 屬 nice-to-have，trade-off 不划算 → **suppress**。`nuxt.config.ts` 設 `experimental.viewTransition: false`；保留 `catalog.css` 的 `view-transition-*` 樣式與元件 `view-transition-name` 綁定（停用後 inert、無害），待 WebKit 修穩可一行 flag 翻回。
- **Red→Green**：`tests/view-transition.test.ts` 與 `tests/nuxt-smoke.test.ts` 原斷言 `experimental.viewTransition === true`，改為 `=== false` 並註明 suppress 原因與本決策連結。
- 驗證：`pnpm test`：13 files、125 tests passed；冷啟動 dev server 後 `http://127.0.0.1:3000/` 與 `https://dwselect.toybox.local/`（Traefik）皆 `HTTP 200`、`.compact-app-shell` 存在、無 `Internal Server Error`/`useHead`/`currentRenderingInstance`/`without provide context`；`#nuxt-ui-colors` 正常存在（colors plugin 已還原為正常運作）。iPad Safari 實測 hydration 穩定。
